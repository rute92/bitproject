...
char webcam_capture()
{
	...
	//static int pic_count = 0, flag = 0; // 계속 멈춰서 사진 찍기 방지
    ...
    int frame_skip, char *prefix, char *out_filename, int mjpeg_port, int json_port, int dont_show, int ext_output, int letter_box_in)
{
    ...
    if (NULL == (from_yolo_fp = fopen(FROM_YOLO_PATH, "w+"))) {
        perror("from_yolo fopen() error\n");
        return 0;
    }

    if (NULL == (to_yolo_fp = fopen(TO_YOLO_PATH, "r+"))) {
        perror("to_yolo fopen() error\n");
        return 0;
    }
    // end code <--
    ...
    while(1){
        ++count;
        {
            ...
            // add code -->
            fread(to_buff, 1, 3, to_yolo_fp); // web 서버 파일로부터 읽기
            fseek(to_yolo_fp, 0, SEEK_SET);
            mode = to_buff[0] - 48;
			manual = to_buff[1] - 48;
            printf("[mode]: %d\n", mode);
			printf("[manual]: %d\n", manual);
			...
			if (mode == 2) { // 일반 청소 모드
				...
				if (manual == 0) { // 수동 조작 모드
					send_buff[1] = to_buff[2];
				}
				else { // tracking 모드
					if (tracking) {
